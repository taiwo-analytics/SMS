...
export default function ParentMessagesPage() {
  ...
  const loadMessages = useCallback(async (currentUserId?: string) => {
    try {
      ...
      if (uid) await loadReadStatus(uid)
    } catch (error) {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const insertData: any = {
        ...
        recipient_role: newMessage.recipient?.trim() || 'admin',
        subject: newMessage.subject,
        content: newMessage.content,
      }
      if (newMessage.reply_to) insertData.reply_to = newMessage.reply_to
  ...
  const handleReply = (message: MessageItem) => {
    ...
    setNewMessage({
      recipient: isFromSomeoneElse ? 'admin' : (message.recipient_role || 'admin'),
